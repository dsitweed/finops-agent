...
Use the matching skill from `.github/skills/` when the task falls within its scope:

- `terraform-skill/SKILL.md` for Terraform/OpenTofu architecture, modules, CI/CD, security, and general IaC decisions.
- `terraform-style-guide/SKILL.md` when generating or reviewing Terraform HCL style and structure.
- `terraform-test/SKILL.md` when writing or troubleshooting `.tftest.hcl` or Terraform test scenarios.
- `fix-issue/SKILL.md` only when the user explicitly asks to implement a GitHub issue; it is marked `disable-model-invocation: true` and should not be activated automatically.

Read the selected `SKILL.md` before implementing the task. Follow the most specific matching skill when multiple skills apply, and use referenced files under that skill directory only when needed.
